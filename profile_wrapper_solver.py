...
def profile_variant(
    ...
    filtered_range_records = filter_recent_iteration_records(final_records, range_last_n)
    range_export_records = filtered_range_records if range_last_n else final_records
    final_ranges = aggregate_selected_node_ranges(range_export_records)
    selected_summary = []
    for node in final_ranges.get("nodes", []):
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=500000,
        help="minimum iterations before early stop can trigger (default: 500000)",
    )
    parser.add_argument(
        "--report-mode",
